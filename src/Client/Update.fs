...
let updateSearchTextMsg msg (model : SearchDetails) =
    ...
    | FetchedSuggestions _, Location -> model, Cmd.none
    | ClearSuggestions, _ -> { model with Suggestions = [||] }, Cmd.none
...
let updateSearchMsg msg model =
    ...
    | SearchTextMsg msg -> updateSearchTextMsg msg model
    ...
    | DeselectProperty -> { model with SelectedProperty = None }, Cmd.none
    | ChangeView view -> 
